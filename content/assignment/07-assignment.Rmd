...
3. Using **forward selection** (that is, select one variable, then select another) create a series of models up to complexity length 15. You may use any variable within the dataset, including categorical variables. While you can code each of the 15 regressions separately, if you really want to be efficient, try to use a loop along with a character vector of your 15 variables to "step" through the 15 regressions. Use a list object to hold your results, and use the `tidy` and `glance` functions from `broom` to extract your `lm` results. 
