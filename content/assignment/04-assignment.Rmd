...
7. Construct a random variable that has expected value $b_2 - b_1$, the pollster bias difference. If our model holds, then this random variable has an approximately normal distribution and we know its standard error. The standard error depends on $\sigma_1$ and $\sigma_2$ (the variances of the $Y$ above), but we can plug the sample standard deviations. __Compute those now__. Now, you should be able to answer the question: is $b_2 - b_1$ different from 0?
