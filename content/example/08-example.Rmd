...
## Task #2
Using the loop method, generate 5 regression trees to explain `wage` in `wage_clean`. You can iterate through values of `cp`, the complexity parameter, or `minsplit`, the minimum # of points that have to be in each split.
...
## Task #3 (if time)
Let's use `lapply` to get a list of your RMSE's (one for each of your models). Note that we are not yet splitting into `test` and `train` (which you **will** need to do on your lab assignment). 
