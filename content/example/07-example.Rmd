...
For this example and discussion, we will briefly use the built in dataset `mtcars` before returning to our favorite `autompg` dataset. During the in-class lecture / example, I will also use much more interesting datasets. The reason to use these easy, straightforward datasets is that they make visualization of the __entire dataset__ trivially easy. Accordingly, the `mtcars` dataset is small, so we'll quickly take a look at the entire dataset.
