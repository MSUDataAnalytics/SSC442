...
**Visual cues** are any element of design that gives the viewer clues as to how to use the object. For instance, we can look at door handles like these
 ...
 ![](/./content/03-content_files/img/pushpull.jpg)
 
 
```{r, warning=FALSE, message=FALSE, fig.align = 'center', fig.subcap='imaginXP', eval =F}
include_graphics(file.path(img_path, 'pushpull.jpg'), error = FALSE)
# ![](/./03-content_files/img/class2_8.jpg)
# # ![](./img/class2_8.jpg)
```
...
include_graphics(file.path(img_path, 'fig8b.png'))
```
...
```{r, eval = F}
include_graphics(file.path(img_path, 'fig1e.png'))
```


```{r, eval = F}
include_graphics(file.path(img_path, 'fig2d.png'))
```
...
```{r, eval = F}
include_graphics(file.path(img_path, 'class2_8.jpg'))
```
...
```{r, eval = F}
include_graphics(file.path(img_path, 'Bush-cuts.png'))
```
...
```{r, eval = F}
include_graphics(file.path(img_path, 'venezuela-election.png'))
...
```{r, eval = F}
include_graphics(file.path(img_path, 'state-of-the-union.png'))
...
```{r, eval = F}
include_graphics(file.path(img_path, 'Colorblind_example.png'))
