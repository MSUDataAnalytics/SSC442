...
linktitle: "3: Visualization II revised"
date: "2021-02-09"
...
Facets are added as an additional layer like this: `+ facet_grid(. ~ sex)`. Since `facet_grid` takes a "formula", all we have to do to facet is decide how we want to lay out our plots. If we want each of the faceting groups to lie along the vertical axis, we put the variable before the "~". If we want the groups to lie along the horizontal axis, we put the variable after the "~". In the example, we'll separate the histogram by drawing them side by side along the horizontal axis.
...
heights %>%
  ...
  facet_grid(.~sex)
```
This would be the result if we took the females, plotted the histogram, then took the males, made another histogram, and then put them side by side.
