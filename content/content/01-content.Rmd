...
2. You must visualize three interesting features of that data.
...
### The Parable of the Playing Cards

Imagine a simplified deck of playing cards with two types of suits: `r emo::ji('club suit')` and `r emo::ji('diamond suit')`.
...
Suppose we do not know the probability distribution within our deck. A sensible thing to do: learn something about $\mu$, the **objective** probability to pick a `r emo::ji('club suit')`.
...
- Nope. It is possible for the sample to be all `r rep(emo::ji('club suit'), 5)` even when the deck is 50/50 `r emo::ji('club suit'); emo::ji('diamond suit')`

  - No matter what we draw, we can't (based on that draw alone) eliminate the possibility of drawing a `r emo::ji('diamond suit')`.
  
  - And unless we assume that the only two values in the world are `r emo::ji('diamond suit')` and `r emo::ji('club suit')`, we can't rule out `r emo::ji('poop')`^[A note from Prof. Kirkpatrick: "I discovered the `emo::ji()` function at 8:55am. My wife joked that I would find a way to use the poop emoji by 9:00am. It is now 8:59am. She was right."]
...
You cannot know anything for sure about $f$ outside the data without making assumptions.This seems like a total mess. Is there any hope to know anything about $f$ outside the data set without making assumptions about $f$?
...
![](/content/01-content_files/data-transformation.pdf)


## Lecture Video
All videos are in the SSC442 Mediaspace channel [available here <i class="fas fa-film"></i>](https://mediaspace.msu.edu/media/SSC442+-+Content+01+-+Jan+18th/1_y8yhmsun?st=385)
